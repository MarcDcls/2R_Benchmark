...
for log in ["logs/R1_random_pwm_22-14-10.json",
            "logs/R1_random_pwm_22-14-28.json",
            "logs/R1_random_pwm_22-14-29.json"]:
        
    plot_logs = open_logs([log])
    net = make_mlp()
    net.load("model.pt")
    ...
    for i in range(state_length):
        ...
        ax.set_title(name)
        ax.scatter(timestamps, real_states[:, i], label=f"real", s=3)
        ax.scatter(timestamps, predicted_values[:, i], label=f"predicted", s=3)
        ax.grid()
        ax.legend()
    ...
    for i in range(action_length):
        ...
        ax.set_title(name)
        ax.scatter(timestamps, real_actions[:, i], label=f"real", s=3)
        ax.grid()
        ax.legend()
